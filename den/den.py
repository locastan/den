...
__version__ = "2.2.06"
__author__ = "Redjumpman, locastan"

log = logging.getLogger("red.den")
_ = Translator("Den", __file__)

deck = Deck()

# Thanks Toby
_DataNamedTuple = namedtuple("Den", "foo")
...
class Data:
    ...
    async def wipe_casino(self, ctx):
        ...
        msg = _("{0.name} ({0.id}) wiped all den data.").format(ctx.author)
        log.info(msg)
        await ctx.send(msg)
...
class Den(Data, BaseCog):
    ...
    async def allin(self, ctx: commands.Context, multiplier: int):
        """Bets all your money for a chance to win big!
    ...
    async def coin(self, ctx: commands.Context, bet: int, choice: str):
        """Dragon Coin flip game with a 50/50 chance to win.

        Pick teeth or tails and place your bet.
        """
        if choice.lower() not in ('teeth', 'tails'):
            return await ctx.send("You must bet teeth or tails.")
    ...
    async def dice(self, ctx: commands.Context, bet: int):
        """Roll a set of bone dice and win on 2, 7, 11, 12.
    ...
    async def craps(self, ctx: commands.Context, bet: int):
        ...
        rolling until they roll a 7 (and lose) or hit their point.
    ...
    async def den(self, ctx: commands.Context):
        """Interacts with the Den system.

        Use help on Den (upper case) for more commands.
        """
        pass
    ...
    async def memberships(self, ctx: commands.Context):
        ...
        msg = await ctx.send(_("Which of the following memberships would you like to know more "
                               "about?\n{}.").format(DenUtils.fmt_join(names)))
        ...
        color = DenUtils.color_lookup(data['Color'])
    ...
    async def wipe(self, ctx: commands.Context):
        """Completely wipes den data."""
        await ctx.send(_("You are about to delete all the den and user data from the bot. Are you "
    ...
    async def admin(self, ctx: commands.Context):
        """A list of Admin level and above commands for the Den."""
        cmds = ('denset access', 'den assignmem', 'denset cooldown',
                'denset max', 'den memprocess', 'denset min', 'denset multiplier',
                'denset name', 'denset payoutlimit', 'denset payouttoggle',
                'den releasecredits', 'den resetinstance', 'den resetuser',
                'den revokemem', 'den wipe')

        cmd_list = '\n'.join(["**{}** - {}".format(x, y) for x, y in
                              [(com, ctx.bot.get_command(com).short_doc) for com in cmds]])

        wiki = '[Den Wiki](https://github.com/Redjumpman/Jumper-Plugins/wiki/Casino-RedV3)'
        embed = discord.Embed(colour=0xFF0000, description=wiki)
        embed.set_author(name='Den Admin Panel', icon_url=ctx.bot.user.avatar_url)
    ...
    async def info(self, ctx: commands.Context):
        """Shows information about the Den.
        ...
        cool = [DenUtils.cooldown_formatter(y["Cooldown"]) for x, y in
    ...
    async def stats(self, ctx: commands.Context):
        """Shows your play statistics for the Den"""
        author = ctx.author
        ...
        color = DenUtils.color_lookup(perks['Color'])
        ...
        fmt_reduct = DenUtils.cooldown_formatter(reduction)
        ...
        embed.title = _("{} Den").format(casino_name)
    ...
    async def version(self, ctx: commands.Context):
        """Shows the current Den version."""
        await ctx.send("The Den is running version {}.".format(__version__))
    ...
    @commands.guild_only()
    @checks.admin_or_permissions(administrator=True)
    async def denset(self, ctx: commands.Context):
        """Changes Den settings"""
        pass

    @denset.command(name='mode')
    @checks.is_owner()
    async def mode(self, ctx: commands.Context):
        """Toggles Den between global and local modes.

        When the den is set to local mode, each server will have its own
        unique data, and admin level commands can be used on that server.

        When the den is set to global mode, data is linked between all servers
        ...
        await ctx.send(_("The Den is currently set to {} mode. Would you like to change to {} "
        ...
        if choice.content.lower() != _('yes'):
            return await ctx.send(_("Den will remain {}.").format(mode))
        await ctx.send(_("Changing the den to {0} will **DELETE ALL** current den data. Are "
                         "you sure you wish to make den {0}?").format(alt))
        try:
        ...
        if final.content.lower() == _('yes'):
            if not await bank.is_global() and alt == "global":
                return await ctx.send("You cannot make den global while economy is "
                                      "in local mode. To change your economy to global "
                                      "use `{}bankset toggleglobal`".format(ctx.prefix))
            await super().change_mode(alt)
            log.info(_("{} ({}) changed the den mode to "
                       "{}.").format(author.name, author.id, alt))
            await ctx.send(_("Den data deleted! Den mode now set to {}.").format(alt))
        else:
            await ctx.send(_("Den will remain {}.").format(mode))
    ...
    async def toggle(self, ctx: commands.Context):
        """Opens and closes the Den for use.
        ...
        msg = _("{0.name} ({0.id}) {2} the {1} "
                "Den.").format(author, name, "closed" if status else "opened")
        log.info(msg)
        await ctx.send(msg)

    @denset.command()
    async def name(self, ctx: commands.Context, *, name: str):
        """Sets the name of the Den.

        The den name may only be 30 characters in length.
        """
        author = ctx.author
        if len(name) > 30:
            return await ctx.send(_("Your Den name must be 30 characters or less."))

        instance = await super().get_instance(ctx, settings=True)
        await instance.Settings.Casino_Name.set(name)
        msg = _("{0.name} ({0.id}) set the den name to {1}.").format(author, name)
        log.info(msg)
        await ctx.send(msg)
    ...
    async def cooldown(self, ctx: commands.Context, game: str, cooldown: str):
        ...
        try:
            seconds = DenUtils.time_converter(cooldown)
        except ValueError:
    ...
    async def access(self, ctx, game: str, access: int):
        ...
        access levels and memberships please refer to the den wiki."""
        author = ctx.author
        instance = await super().get_instance(ctx, settings=True)
        games = await instance.Games.all()
    ...
    async def process_user(self, memberships, user, _global=False):
        ...
        try:
            ...
            raise RuntimeError("Den is in global mode, while economy is in local mode. "
                               "Economy must be global if he Den is global. Either change den "
    ...
    def parse_cooldowns(ctx, cooldowns, reduction):
        ...
        for cooldown in cooldowns:
            ...
            results.append(DenUtils.cooldown_formatter(seconds, custom_msg="<<Ready to Play!"))
        return results
...
class Engine(Data):
    ...
    async def check_conditions(self):
        ...
        if self.choices is not None and self.choice not in self.choices:
            error = _("Incorrect response. Accepted response are:"
                      "\n{}.").format(DenUtils.fmt_join(self.choices))
        elif not guild_data["Settings"]["Casino_Open"]:
            error = _("The Den is closed.")
    ...
    async def check_cooldown(self, game_data, user_instance):
        ...
        if now >= user_time - reduction:
            ...
            remaining = DenUtils.time_formatter(seconds)
    ...
    async def game_teardown(self, result):
        ...
        embed.title = _("{} Den | {}").format(casino_name, self.game)
        if not win:
        ...
        if self.game == 'Allin':
            ...
            total -= self.bet #deduct initial bet from winning to show net win.
    ...
    async def calculate_bonus(amount, player_instance, coro):
        ...
        bonus = total - amount
        return total, amount, "(+{} Bonus)".format(bonus if bonus_multiplier > 1 else 0)
...
class Membership(Data):
    ...
    async def delete(self):
        ...
        await self.ctx.send(_("Which membership would you like to delete?\n"
                              "{}.").format(DenUtils.fmt_join(memberships)))
    ...
    async def editor(self):
        ...
        await self.ctx.send(_("Which of the following attributes would you like to edit?\n"
                              "{}.").format(DenUtils.fmt_join(attrs)))
    ...
    async def set_color(self, membership):
        await self.ctx.send(_("What color would you like to set?\n"
                              "{}").format(DenUtils.fmt_join(list(self.colors))))
    ...
    async def req_loop(self, membership):
        while True:
            await self.ctx.send(_("Which requirement would you like to add or modify?\n"
                                  "{}.").format(DenUtils.fmt_join(self.requirements)))
...
class Core:
    """
    A simple class to hold the basic original Den mini games.
    ...
    async def play_allin(self, ctx, bet, multiplier):
        ...
        embed = DenUtils.build_embed(msg)
        return outcome == 0, bet, embed

    @game_engine("Dragon Coin", (_("teeth"), _("tails")))
    async def play_coin(self, ctx, bet, choice):
        await ctx.send(_("The dragon coin flips into the air..."))
        await asyncio.sleep(2)
        outcome = random.choice((_("teeth"), _("tails")))
        embed = DenUtils.build_embed(_("The coin landed on {}!").format(outcome))
    ...
    async def play_cups(self, ctx, bet, choice):
        ...
        embed = DenUtils.build_embed(_("The coin was under cup {}!").format(outcome))
        return int(choice) == outcome, bet, embed

    @game_engine("Dice")
    async def play_dice(self, ctx, bet):
        await ctx.send(_("The bone dice strike the back of the table and begin to tumble into "
                         "place..."))
        await asyncio.sleep(2)
...
class Craps:
    ...
    async def craps_game(self, ctx, bet):
        ...
        comeout = d1 + d2
        msg = _("Your comeout was a {} and {}.")

        if comeout == 7:
            bet *= 7
            return True, bet, DenUtils.build_embed(msg.format(d1, d2))
        elif comeout == 11:
            return True, bet, DenUtils.build_embed(msg.format(d1, d2))
        elif comeout in (2, 3, 12):
            return False, bet, DenUtils.build_embed(msg.format(d1, d2))
    ...
    async def point_loop(self, ctx, bet, comeout, roll_msg, msg):
        ...
        while True:
            ...
            if (d1 + d2) == comeout or (d1 + d2) in (7, 11) or count >= 3:
                ...
                return (d1 + d2) == comeout, bet, DenUtils.build_embed(msg.format(d1, d2))
            else:
